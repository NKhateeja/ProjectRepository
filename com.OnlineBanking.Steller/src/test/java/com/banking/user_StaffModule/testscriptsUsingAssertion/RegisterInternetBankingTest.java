...
//		Eut.insertDataIntoExcel("OpenAccount", 2, 19, ApplicationNo);
		...
		sa.assertTrue(Wut.getUrl(driver).contains("ebanking"));
		...
		sa.assertTrue(Wut.getUrl(driver).contains("customer_login")); 
		...
		Reporter.log("RegisterInternetBankingTest testcase is passed",true);
		sa.assertAll();
